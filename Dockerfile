...
RUN CGO_ENABLED=0 GOOS=linux GOARCH=amd64 \
  ...
  -o /verification-agent-binaries/BackupForge-verification-agent-linux-amd64 ./cmd

RUN CGO_ENABLED=0 GOOS=linux GOARCH=arm64 \
  go build -ldflags "-X main.Version=${APP_VERSION}" \
  -o /verification-agent-binaries/BackupForge-verification-agent-linux-arm64 ./cmd
...
RUN groupadd -g 999 postgres || true && \
  ...
  mkdir -p /BackupForge-data/pgdata && \
  chown -R postgres:postgres /BackupForge-data/pgdata

# Create non-root user for the main application process
RUN useradd -r -s /usr/sbin/nologin -u 65532 BackupForge
...
# Expose the binary as the `BackupForge` command on PATH (e.g. `BackupForge healthcheck`)
RUN ln -s /app/main /usr/local/bin/BackupForge
...
if [ -d "/postgresus-data" ] && [ "\$(ls -A /postgresus-data 2>/dev/null)" ]; then
    ...
    echo "=========================================="
    echo ""
    echo "You are using the \`postgresus-data\` folder. It seems you changed the image name from Postgresus to BackupForge without changing the volume."
    echo ""
    echo "Please either:"
    echo "  1. Switch back to image rostislavdugin/postgresus:latest (supported until ~Dec 2026)"
    echo "  2. Read the migration guide: https://BackupForge.com/installation/#postgresus-migration"
    echo ""
    echo "=========================================="
    exit 1
fi
...
# Detect if email is configured (both SMTP_HOST and BackupForge_URL must be set)
if [ -n "\${SMTP_HOST:-}" ] && [ -n "\${BackupForge_URL:-}" ]; then
...
if [ -n "\${ANALYTICS_SCRIPT:-}" ]; then
  if ! grep -q "rybbit.BackupForge.com" /app/ui/build/index.html 2>/dev/null; then
...
mkdir -p /BackupForge-data/pgdata
mkdir -p /BackupForge-data/temp
mkdir -p /BackupForge-data/backups
chown BackupForge:BackupForge /BackupForge-data
chown -R postgres:postgres /BackupForge-data/pgdata
chown -R BackupForge:BackupForge /BackupForge-data/temp /BackupForge-data/backups
...
chown BackupForge:BackupForge /BackupForge-data/secret.key /BackupForge-data/instance.json 2>/dev/null || true
chmod 700 /BackupForge-data/temp
...
if [ ! -s "/BackupForge-data/pgdata/PG_VERSION" ]; then
    echo "Initializing PostgreSQL database..."
    gosu postgres \$PG_BIN/initdb -D /BackupForge-data/pgdata --encoding=UTF8 --locale=C.UTF-8
    
    # Configure PostgreSQL
    echo "host all all 127.0.0.1/32 md5" >> /BackupForge-data/pgdata/pg_hba.conf
    echo "local all all trust" >> /BackupForge-data/pgdata/pg_hba.conf
    echo "port = 5437" >> /BackupForge-data/pgdata/postgresql.conf
    echo "listen_addresses = 'localhost'" >> /BackupForge-data/pgdata/postgresql.conf
    echo "shared_buffers = 256MB" >> /BackupForge-data/pgdata/postgresql.conf
    echo "max_connections = 100" >> /BackupForge-data/pgdata/postgresql.conf
fi
...
start_postgres() {
    ...
    gosu postgres \$PG_BIN/postgres -D /BackupForge-data/pgdata -p 5437 -k /tmp &
    POSTGRES_PID=\$!
...
if ! start_postgres; then
    ...
    if gosu postgres \$PG_BIN/pg_resetwal -f /BackupForge-data/pgdata; then
        ...
        if start_postgres; then
            ...
            echo "  2. Manually inspect /BackupForge-data/pgdata for issues"
            echo "=========================================="
            exit 1
        fi
    else
...
SELECT 'CREATE DATABASE BackupForge OWNER postgres'
WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname = 'BackupForge')
\\gexec
\\q
SQL
...
WAL_CHECK_DSN="postgres://postgres:Q1234567@localhost:5437/BackupForge"

WAL_CHECK_COL=\$(gosu BackupForge \$PG_BIN/psql "\$WAL_CHECK_DSN" -tA -c "SELECT 1 FROM information_schema.columns WHERE table_name='postgresql_databases' AND column_name='backup_type' LIMIT 1" 2>/dev/null || true)

if [ "\$WAL_CHECK_COL" = "1" ]; then
    WAL_CHECK_ROW=\$(gosu BackupForge \$PG_BIN/psql "\$WAL_CHECK_DSN" -tA -c "SELECT 1 FROM postgresql_databases WHERE backup_type='WAL_V1' LIMIT 1" 2>/dev/null || true)
...
echo "Starting BackupForge application..."

exec gosu BackupForge ./main
EOF

LABEL org.opencontainers.image.source="https://github.com/BackupForge/BackupForge"
...
HEALTHCHECK --interval=30s --timeout=5s --start-period=60s --retries=3 \
  CMD ["BackupForge", "healthcheck"]

# Volume for PostgreSQL data
VOLUME ["/BackupForge-data"]
