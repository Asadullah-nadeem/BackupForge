...
var verificationAgentBinaryPaths = map[string]string{
	"amd64": "agent-binaries/BackupForge-verification-agent-linux-amd64",
	"arm64": "agent-binaries/BackupForge-verification-agent-linux-arm64",
}

// DownloadVerificationAgent
// @Summary Download verification agent binary
// @Description Download the BackupForge-verification-agent binary for the specified architecture
// @Tags system/agent
// @Produce octet-stream
// @Param arch query string true "Target architecture" Enums(amd64, arm64)
// @Success 200 {file} binary
// @Header 200 {string} X-BackupForge-Version "Version the served binary was built from"
...
func (c *AgentController) DownloadVerificationAgent(ctx *gin.Context) {
	...
	ctx.Header("Content-Disposition", "attachment; filename=BackupForge-verification-agent")
