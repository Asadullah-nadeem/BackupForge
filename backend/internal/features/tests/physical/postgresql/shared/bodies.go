...
// BackupForge streams WAL from a standby, the standby is promoted to a new timeline mid-stream, and
...
func RunWalStreamCatalogsHistoryOnTimelineSwitch(t *testing.T, version, image string) {
	...
	// BackupForge backs up the STANDBY: pg_basebackup and pg_receivewal run against it, so promoting it
