...
import (
	...
	backups_controllers_physical "BackupForge-backend/internal/features/backups/backups/controllers/physical"
	backups_core_enums "BackupForge-backend/internal/features/backups/backups/core/enums"
	"BackupForge-backend/internal/features/backups/backups/core/physical/chain_view"
	physical_enums "BackupForge-backend/internal/features/backups/backups/core/physical/enums"
	physical_repositories "BackupForge-backend/internal/features/backups/backups/core/physical/repositories"
	backups_dto_physical "BackupForge-backend/internal/features/backups/backups/dto/physical"
	postgresql_executor "BackupForge-backend/internal/features/backups/backups/usecases/physical/postgresql"
	...
	postgresql_physical "BackupForge-backend/internal/features/databases/databases/postgresql/physical"
	"BackupForge-backend/internal/features/intervals"
	"BackupForge-backend/internal/features/notifiers"
	"BackupForge-backend/internal/features/storages"
	users_middleware "BackupForge-backend/internal/features/users/middleware"
	...
	test_utils "BackupForge-backend/internal/util/testing"
	"BackupForge-backend/internal/util/testing/containers"
	"BackupForge-backend/internal/util/walmath"
)
...
// remains — the persistent WAL streamer slot (BackupForge_slot_*) and any transient
// per-backup slot (BackupForge_basebackup_*) — proving the deleted database left
...
func reconstructCluster(
	...
	stagedScript := "/tmp/BackupForge-recovery.sh"
...
func fetchRecoveryScript(t *testing.T, router *gin.Engine) string {
	...
	hostPath := filepath.Join(t.TempDir(), "BackupForge-recovery.sh")
...
func requireRestoredClusterNeedsNoZstd(t *testing.T, target containers.RestoreTarget, image string) {
	...
	leftoverCompressed := strings.TrimSpace(string(
		target.Exec(t, "sh", "-c", "ls "+clusterDir+"/BackupForge_wal_restore/*.zst 2>/dev/null | wc -l")))
...
// connection targets the standby BackupForge backs up.
