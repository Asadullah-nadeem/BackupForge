#!/bin/sh
# BackupForge physical-restore helper.
#
# Takes a restore bundle produced by the BackupForge restore stream, decompresses
...
#
# Usage:
#   sh BackupForge-recovery.sh [--pg-bin <dir>] [--combine-image <image>] [--target-time <utc-ts>] <bundle-url-or-path> [output-dir]
#
...
#   [output-dir]          where to build the restore (default: BackupForge-restore).

set -eu

usage() {
    cat >&2 <<'USAGE'
usage: sh BackupForge-recovery.sh [--pg-bin <dir>] [--combine-image <image>] [--target-time <utc-ts>] <bundle-url-or-path> [output-dir]
...
if [ -d "$WAL_DIR" ] && [ -n "$(ls -A "$WAL_DIR" 2>/dev/null)" ]; then
    ...
    PLAIN_WAL_DIR="$DATA_DIR/BackupForge_wal_restore"
    ...
    restore_command="cp \"BackupForge_wal_restore/%f\" \"%p\""
    ...
    if [ -n "$MAX_CONNECTIONS$MAX_WORKER_PROCESSES$MAX_WAL_SENDERS$MAX_PREPARED_XACTS$MAX_LOCKS_PER_XACT" ]; then
        {
            echo "# BackupForge restore: WAL replay needs these >= the primary's values (read from the"
