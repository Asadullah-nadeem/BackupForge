...
import (
	...
	"BackupForge-backend/internal/util/testing/mailpit"
)
...
func Test_EmailSMTPSenderSendEmail_WhenSmtpServerAccepts_DeliversMessageToRecipient(t *testing.T) {
	...
	sender := &EmailSMTPSender{
		...
		smtpFrom:     "sender@BackupForge.local",
		isConfigured: true,
	}

	err := sender.SendEmail("recipient@BackupForge.local", "Password Reset Code", "<b>123456</b>")
	...
	assert.Equal(t, "recipient@BackupForge.local", delivered[0].To[0].Address)
}
