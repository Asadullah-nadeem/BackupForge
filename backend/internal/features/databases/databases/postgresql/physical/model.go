...
func (p *PostgresqlPhysicalDatabase) BeforeCreate(_ *gorm.DB) error {
	if p.ReplicationSlotName == "" {
		p.ReplicationSlotName = "BackupForge_slot_" + strings.ReplaceAll(uuid.New().String(), "-", "")
	}
...
// name (BackupForge_slot_<uuid>) is owned exclusively by this database — waits for the
...
func (p *PostgresqlPhysicalDatabase) CreateReplicationOnlyUser(
	...
	for attempt := range maxRetries {
		baseUsername := "BackupForge-" + uuid.New().String()[:8]
