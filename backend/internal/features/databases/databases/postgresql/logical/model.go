...
func (p *PostgresqlLogicalDatabase) Validate() error {
	...
	// Prevent BackupForge from backing up itself
	// BackupForge runs an internal PostgreSQL instance that should not be backed up through the UI
	// because it would expose internal metadata to non-system administrators.
	// To properly backup BackupForge, see: https://BackupForge.com/faq#backup-BackupForge
	if p.Database != nil && *p.Database != "" {
		...
		if isLocalhost && strings.EqualFold(*p.Database, "BackupForge") {
			return errors.New(
				"backing up BackupForge internal database is not allowed. To backup BackupForge itself, see https://BackupForge.com/faq#backup-BackupForge",
			)
		}
	}
...
//
// Security features:
// - Username format: "BackupForge-{8-char-uuid}" for uniqueness
...
func (p *PostgresqlLogicalDatabase) CreateReadOnlyUser(
	...
	for attempt := range maxRetries {
		...
		baseUsername := fmt.Sprintf("BackupForge-%s", uuid.New().String()[:8])
