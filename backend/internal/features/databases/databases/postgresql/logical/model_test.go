...
import (
	...
	postgresql_shared "BackupForge-backend/internal/features/databases/databases/postgresql/shared"
	"BackupForge-backend/internal/util/testing/containers"
...
func testCreateReadOnlyUserCanReadButNotWrite(t *testing.T, endpoint containers.Endpoint, version string) {
	...
	assert.True(t, strings.HasPrefix(username, "BackupForge-"))
...
func Test_Validate_WhenLocalhostAndBackupForge_ReturnsError(t *testing.T) {
	testCases := []struct {
		name     string
		host     string
		username string
		...
		{
			name:     "localhost with BackupForge db",
			host:     "localhost",
			username: "postgres",
			database: "BackupForge",
		},
		{
			name:     "127.0.0.1 with BackupForge db",
			host:     "127.0.0.1",
			username: "postgres",
			database: "BackupForge",
		},
		{
			name:     "172.17.0.1 with BackupForge db",
			host:     "172.17.0.1",
			username: "postgres",
			database: "BackupForge",
		},
		{
			name:     "host.docker.internal with BackupForge db",
			host:     "host.docker.internal",
			username: "postgres",
			database: "BackupForge",
		},
		{
			name:     "LOCALHOST (uppercase) with BackupForge db",
			host:     "LOCALHOST",
			username: "POSTGRES",
			database: "BackupForge",
		},
		{
			name:     "LocalHost (mixed case) with BackupForge db",
			host:     "LocalHost",
			username: "anyuser",
			database: "BackupForge",
		},
		{
			name:     "localhost with BackupForge and any username",
			host:     "localhost",
			username: "myuser",
			database: "BackupForge",
		},
		{
			name:     "::1 (IPv6 loopback) with BackupForge db",
			host:     "::1",
			username: "postgres",
			database: "BackupForge",
		},
		{
			name:     ":: (IPv6 all interfaces) with BackupForge db",
			host:     "::",
			username: "postgres",
			database: "BackupForge",
		},
		{
			name:     "::1 (uppercase) with BackupForge db",
			host:     "::1",
			username: "POSTGRES",
			database: "BackupForge",
		},
		{
			name:     "0.0.0.0 (all IPv4 interfaces) with BackupForge db",
			host:     "0.0.0.0",
			username: "postgres",
			database: "BackupForge",
		},
		{
			name:     "127.0.0.2 (loopback range) with BackupForge db",
			host:     "127.0.0.2",
			username: "postgres",
			database: "BackupForge",
		},
		{
			name:     "127.255.255.255 (end of loopback range) with BackupForge db",
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			assert.Error(t, err)
			assert.Contains(t, err.Error(), "backing up BackupForge internal database is not allowed")
			assert.Contains(t, err.Error(), "https://BackupForge.com/faq#backup-BackupForge")
		})
	}
}

func Test_Validate_WhenNotLocalhostOrNotBackupForge_ValidatesSuccessfully(t *testing.T) {
	testCases := []struct {
		name     string
		host     string
		username string
		...
		{
			name:     "different host (remote server) with BackupForge db",
			host:     "192.168.1.100",
			username: "postgres",
			database: "BackupForge",
		},
		...
		{
			name:     "remote host with BackupForge db name (allowed)",
			host:     "db.example.com",
			username: "postgres",
			database: "BackupForge",
		},
...
func Test_CreateReadOnlyUser_TablesCreatedByDifferentUser_ReadOnlyUserCanRead(t *testing.T) {
	...
	// Step 3: NOW create read-only user via BackupForge (as admin)
