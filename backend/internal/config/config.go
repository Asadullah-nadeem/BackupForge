...
type EnvVariables struct {
	...
	BackupForgeURL string `env:"BackupForge_URL"`
}
...
func loadEnvVariables() {
	...
	// (projectRoot/BackupForge-data -> /BackupForge-data)
	env.DataFolder = filepath.Join(filepath.Dir(backendRoot), "BackupForge-data", "backups")
	env.TempFolder = filepath.Join(filepath.Dir(backendRoot), "BackupForge-data", "temp")
	env.SecretKeyPath = filepath.Join(filepath.Dir(backendRoot), "BackupForge-data", "secret.key")
	env.TelemetryInstancePath = filepath.Join(
		filepath.Dir(backendRoot), "BackupForge-data", "instance.json",
	)
