...
const physicalPrimaryAlias = "BackupForge-primary"

// physicalStandbySlotName is the physical replication slot the standby creates on the primary to
// pin WAL until it has streamed it. Distinct from the BackupForge_* slots BackupForge manages on the
// node it backs up.
const physicalStandbySlotName = "BackupForge_test_standby"
...
func startPhysicalStandbyOnNetwork(
	...
	req := testcontainers.ContainerRequest{
		...
		ExposedPorts: []string{postgresPort},
		Env:          postgresEnv(),
		Entrypoint:   []string{"/usr/local/bin/BackupForge-standby-entrypoint.sh"},
		Cmd:          physicalPostgresCmd(options),
		Files: []testcontainers.ContainerFile{{
			Reader:            strings.NewReader(standbyEntrypointScript()),
			ContainerFilePath: "/usr/local/bin/BackupForge-standby-entrypoint.sh",
			FileMode:          0o755,
		}},
		Tmpfs:      map[string]string{postgresDataDir(image): physicalDataDirTmpfsOptions},
		Networks:   []string{networkName},
...
echo "BackupForge-standby: waiting for primary to accept connections"
until pg_isready -h ` + physicalPrimaryAlias + ` -p 5432 -U ` + PostgresUsername + ` >/dev/null 2>&1; do sleep 1; done
echo "BackupForge-standby: cloning primary via pg_basebackup"
...
echo "BackupForge-standby: starting as a streaming standby"
...
func StartPhysicalPostgresMtls(t *testing.T, contextDir string) Endpoint {
	...
	return startPostgresBuild(t, contextDir, "BackupForge-test-postgres-physical-mtls")
}
...
func StartPhysicalRestoreTarget(t *testing.T, image string) RestoreTarget {
	...
	req := testcontainers.ContainerRequest{
		FromDockerfile: testcontainers.FromDockerfile{
			Context:   contextDir,
			Repo:      "BackupForge-test-postgres-restore-target",
