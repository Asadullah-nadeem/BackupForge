...
//
//  1. The BackupForge metadata DB: drop and recreate the test schema referenced
...
// (BackupForge_test_w0 .. BackupForge_test_w{pool-1}). Tests run with `go test -p`,
...
func resetOnePostgresContainer(ctx context.Context, log *slog.Logger, host, label, port string) error {
	...
	if err := dropBackupForgeReplicationSlots(ctx, log, db); err != nil {
		return fmt.Errorf("drop BackupForge replication slots: %w", err)
	}
...
// dropBackupForgeReplicationSlots removes every replication slot whose name
// starts with the BackupForge prefixes. Idempotent: a missing slot returns
// pgx.ErrNoRows from the subquery and the SELECT yields no rows.
func dropBackupForgeReplicationSlots(ctx context.Context, log *slog.Logger, db *sql.DB) error {
	rows, err := db.QueryContext(ctx, `
		SELECT slot_name FROM pg_replication_slots
		WHERE slot_name LIKE 'BackupForge_basebackup_%'
		   OR slot_name LIKE 'BackupForge_slot_%'
	`)
	if err != nil {
