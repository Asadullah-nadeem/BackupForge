...
import (
	...
	"BackupForge-backend/internal/features/backups/backups/backuping/logical"
	...
	healthcheck_attempt "BackupForge-backend/internal/features/healthcheck/attempt"
	...
	"BackupForge-backend/internal/features/restores/restoring"
	"BackupForge-backend/internal/features/storages"
	system_agent "BackupForge-backend/internal/features/system/agent"
	system_healthcheck "BackupForge-backend/internal/features/system/healthcheck"
	...
	"BackupForge-backend/internal/features/telemetry"
	users_controllers "BackupForge-backend/internal/features/users/controllers"
	...
	"BackupForge-backend/internal/middleware"
	cache_utils "BackupForge-backend/internal/util/cache"
	env_utils "BackupForge-backend/internal/util/env"
	files_utils "BackupForge-backend/internal/util/files"
	"BackupForge-backend/internal/util/logger"
	_ "BackupForge-backend/swagger" // swagger docs
)

// @title BackupForgege Backend API
// @version 1.0
// @description API for BackupForgege
...
func startServerWithGracefulShutdown(log *slog.Logger, app *gin.Engine) {
	...
	if runtime.GOOS == "windows" {
		go func() {
			...
			log.Info("Launching BackupForge Desktop window...")
			_ = exec.Command("cmd", "/c", "start", "msedge", "--app=http://localhost:4005").Run()
		}()
	}
...
func announceTelemetry() {
	...
	fmt.Println(
		"Anonymous telemetry collected (BackupForge version, OS/arch, etc.). No DB contents, no user data. " +
