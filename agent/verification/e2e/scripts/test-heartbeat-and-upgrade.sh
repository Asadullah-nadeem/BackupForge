...
INITIAL="$(./BackupForge-verification-agent version)"
...
./BackupForge-verification-agent run \
...
while [ $SECONDS -lt $DEADLINE ]; do
  [ "$(./BackupForge-verification-agent version)" = "v2.0.0" ] && break
  sleep 3
done

FINAL="$(./BackupForge-verification-agent version)"
