...
dump_daemon_log() {
  echo "---- BackupForge-verification.daemon.log ----"
  cat BackupForge-verification.daemon.log 2>/dev/null || echo "(no daemon log)"
}
...
cp "$ARTIFACTS/agent-v1" ./BackupForge-verification-agent
chmod +x ./BackupForge-verification-agent

LAUNCH_FLAGS=(
  --BackupForge-host "$MOCK"
  --agent-id "$AGENT_ID"
  --token test-token
  --max-cpu 4
  --max-ram-mb 2048
...
STATUS_OUT="$(./BackupForge-verification-agent status)"
...
START_OUT="$(./BackupForge-verification-agent start "${LAUNCH_FLAGS[@]}")"
...
if [ ! -f BackupForge-verification.lock ]; then
  echo "FAIL: BackupForge-verification.lock not created"
  dump_daemon_log
  exit 1
fi
...
set +e
SECOND_OUT="$(./BackupForge-verification-agent run "${LAUNCH_FLAGS[@]}" 2>&1)"
SECOND_RC=$?
set -e
...
./BackupForge-verification-agent stop
GONE=0
