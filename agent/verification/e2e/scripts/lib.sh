...
leak_check() {
  local agent_id="$1"
  local label="BackupForge.verification.agent_id=${agent_id}"
  local rc=0
...
# start_agent <agent_id> [extra_flag...] — runs `BackupForge-verification-agent run`
...
dump_diagnostics() {
  ...
  echo "---- BackupForge-verification.log (last 60) ----"
  tail -60 BackupForge-verification.log 2>/dev/null || echo "(no log file)"
}
