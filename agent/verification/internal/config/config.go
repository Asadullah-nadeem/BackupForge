...
import (
	...
	"BackupForge-verification-agent/internal/logger"
)

var log = logger.GetLogger()

const configFileName = "BackupForge-verification.json"
...
const DefaultVerificationPgImageRepo = "BackupForge/verification-postgres"

type Config struct {
	BackupForgeHost           string `json:"BackupForgeHost"`
...
// LoadFromJSONAndArgs reads BackupForge-verification.json into the struct
...
// SaveToJSON writes the current struct to BackupForge-verification.json.
...
// LoadFromJSON reads BackupForge-verification.json without applying CLI flags.
...
func (c *Config) Validate() (Capacity, error) {
	if c.BackupForgeHost == "" {
		return Capacity{}, fmt.Errorf("BackupForge-host is required")
	}
...
func (c *Config) ValidateTransport(isStdinTTY bool, in *bufio.Reader) error {
	parsed, err := url.Parse(c.BackupForgeHost)
	if err != nil {
		return fmt.Errorf("BackupForge-host is not a valid URL: %w", err)
	}
	...
	default:
		return fmt.Errorf(
			"BackupForge-host must start with https:// or http:// (got scheme %q)", parsed.Scheme)
	}
}
...
func (c *Config) loadFromJSON() {
	...
	if err != nil {
		if os.IsNotExist(err) {
			log.Info("No BackupForge-verification.json found, will create on save")
			return
		}

		log.Warn("Failed to read BackupForge-verification.json", "error", err)

		return
	}

	if err := json.Unmarshal(data, c); err != nil {
		log.Warn("Failed to parse BackupForge-verification.json", "error", err)
...
func (c *Config) consentToInsecureHTTP(isStdinTTY bool, in *bufio.Reader) error {
	if c.AllowInsecureHTTP {
		log.Warn("BackupForge-host is plain HTTP; transport is unencrypted")
	...
	if !isStdinTTY {
		return fmt.Errorf(
			"refusing to use plain HTTP over a non-interactive connection: " +
				"switch --BackupForge-host to https:// or pass --allow-insecure-http " +
