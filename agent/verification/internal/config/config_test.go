...
var allFlagNames = []string{
	"BackupForge-host", "agent-id", "token", "max-cpu", "max-ram-mb",
...
func Test_Validate_WhenConfigComplete_ReturnsCapacity(t *testing.T) {
	cfg := &Config{
		BackupForgeHost:     "https://primary.example:4005",
		AgentID:           "agent-1",
		Token:             "secret",
		MaxCPU:            8,
		MaxRAMMb:          4096,
...
func Test_Validate_WhenRequiredStringMissing_ReturnsError(t *testing.T) {
	for name, cfg := range map[string]*Config{
		"no BackupForge-host": {AgentID: "a", Token: "t", MaxCPU: 4, MaxRAMMb: 2048, MaxDiskGb: 10, MaxConcurrentJobs: 1},
		"no agent-id":       {BackupForgeHost: "https://x:4005", Token: "t", MaxCPU: 4, MaxRAMMb: 2048, MaxDiskGb: 10, MaxConcurrentJobs: 1},
		"no token":          {BackupForgeHost: "https://x:4005", AgentID: "a", MaxCPU: 4, MaxRAMMb: 2048, MaxDiskGb: 10, MaxConcurrentJobs: 1},
	} {
...
func Test_ValidateTransport_WhenHTTPS_PassesWithoutPrompt(t *testing.T) {
	cfg := &Config{BackupForgeHost: "https://primary.example:4005"}
...
func Test_ValidateTransport_WhenHTTPNonTTYWithoutFlag_FailsNamingBothFixes(t *testing.T) {
	cfg := &Config{BackupForgeHost: "http://primary.example:4005"}
...
func Test_ValidateTransport_WhenHTTPWithAllowFlag_PassesWithWarn(t *testing.T) {
	cfg := &Config{BackupForgeHost: "http://primary.example:4005", AllowInsecureHTTP: true}
...
func Test_ValidateTransport_WhenSchemeUnsupported_ReturnsError(t *testing.T) {
	cfg := &Config{BackupForgeHost: "ftp://primary.example:4005"}
...
func Test_LoadFromJSONAndArgs_WhenFlagsProvided_OverrideEveryValue(t *testing.T) {
	...
	cfg := loadConfig(t,
		"--BackupForge-host", "https://flag:4005",
		"--agent-id", "agent-x",
		"--token", "tok",
		"--max-cpu", "8",
		"--max-ram-mb", "4096",
	...
	assert.Equal(t, "https://flag:4005", cfg.BackupForgeHost)
...
func Test_LoadFromJSONAndArgs_WhenOnlyFilePresent_LoadsFileAndMarksSource(t *testing.T) {
	t.Chdir(t.TempDir())
	source := &Config{BackupForgeHost: "https://file:4005", AgentID: "agent-file", MaxCPU: 6}
	require.NoError(t, source.SaveToJSON())

	cfg := loadConfig(t)

	assert.Equal(t, "https://file:4005", cfg.BackupForgeHost)
	assert.Equal(t, "agent-file", cfg.AgentID)
	assert.Equal(t, 6, cfg.MaxCPU)
	assert.Equal(t, configFileName, cfg.flagSources["BackupForge-host"])
}

func Test_LoadFromJSONAndArgs_WhenFlagAndFile_FlagOverridesFile(t *testing.T) {
	t.Chdir(t.TempDir())
	source := &Config{BackupForgeHost: "https://file:4005", AgentID: "agent-file"}
	require.NoError(t, source.SaveToJSON())

	cfg := loadConfig(t, "--BackupForge-host", "https://flag:4005")

	assert.Equal(t, "https://flag:4005", cfg.BackupForgeHost, "flag overrides file")
	assert.Equal(t, "agent-file", cfg.AgentID, "untouched field keeps the file value")
	assert.Equal(t, "command line args", cfg.flagSources["BackupForge-host"])
