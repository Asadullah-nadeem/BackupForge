...
func (c *Config) flagBindings() []flagBinding {
	return []flagBinding{
		stringBinding("BackupForge-host", "BackupForge server URL (e.g. https://your-server:4005)",
			func(c *Config) string { return c.BackupForgeHost },
			func(c *Config, v string) { c.BackupForgeHost = v }),
		...
		boolBinding("allow-insecure-http",
			"Permit a plain http:// BackupForge-host (token and backup data sent unencrypted)",
