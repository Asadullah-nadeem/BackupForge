...
import (
	...
	"BackupForge-verification-agent/internal/testutil"
)

func Test_BuildSpec_AppliesHardeningControls(t *testing.T) {
	// nil engine: buildSpec is pure and never calls the engine.
	containerManager := NewManager(nil, "agent-1", "BackupForge/verification-postgres", testutil.DiscardLogger())
...
func Test_ImageForMajor_AppendsMajorTagToConfiguredRepo(t *testing.T) {
	cases := []struct {
		...
		{"12", "BackupForge/verification-postgres:12"},
		{"16", "BackupForge/verification-postgres:16"},
		{"18", "BackupForge/verification-postgres:18"},
	}

	for _, tc := range cases {
		t.Run(tc.major, func(t *testing.T) {
			assert.Equal(t, tc.expected, imageForMajor(tc.major, "BackupForge/verification-postgres"))
		})
	}
}
...
func Test_ImageForJob_WhenTimescaleVersionSet_ReturnsVersionMatchedTimescaleImage(t *testing.T) {
	image := imageForJob(
		SpawnRequest{PgMajor: "17", TimescaledbVersion: "2.17.0"}, "BackupForge/verification-postgres")
...
func Test_ImageForJob_WhenNoTimescaleVersion_UsesConfiguredEngineRepo(t *testing.T) {
	image := imageForJob(SpawnRequest{PgMajor: "17"}, "BackupForge/verification-postgres")

	assert.Equal(t, "BackupForge/verification-postgres:17", image)
}
...
func Test_StockFallbackImage_WhenBundledEngineRepo_FallsBackToStockPostgres(t *testing.T) {
	fallbackImage, canFallBack := stockFallbackImage(
		SpawnRequest{PgMajor: "16"}, "BackupForge/verification-postgres:16")
