...
if [ "$(id -u)" -ne 0 ]; then
    echo "Error: This script must be run as root (sudo ./install-BackupForge.sh)" >&2
    exit 1
fi

# Set up logging
LOG_FILE="/var/log/BackupForge-install.log"
INSTALL_DIR="/opt/BackupForge"
...
touch "$LOG_FILE"
log "Starting BackupForge installation..."
...
services:
  BackupForge:
    container_name: BackupForge
    image: BackupForge/BackupForge:latest
    ports:
      - "4005:4005"
    volumes:
      - ./BackupForge-data:/BackupForge-data
    restart: unless-stopped
    healthcheck:
      test: ["CMD", "BackupForge", "healthcheck"]
      interval: 30s
      timeout: 5s
      retries: 3
      start_period: 60s
...
# Start BackupForge
log "Starting BackupForge..."
cd "$INSTALL_DIR"
if docker compose up -d; then
    log "BackupForge started successfully"
else
    log "ERROR: Failed to start BackupForge!"
    exit 1
fi

log "BackupForge installation completed successfully!"
log "-------------------------------------------"
log "To launch:"
log "> cd $INSTALL_DIR && docker compose up -d"
log "Access BackupForge at: http://localhost:4005"
