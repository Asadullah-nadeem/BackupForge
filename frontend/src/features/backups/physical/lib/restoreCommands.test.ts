...
describe('buildScriptCommand', () => {
  it('omits --pg-bin and --target-time when neither is set', () => {
    ...
    expect(command).toContain(`"${bundleUrl}" "./BackupForge-restore"`);
  });
...
describe('buildManualSteps', () => {
  ...
  it('adds a recovery-parameter note pointing at pg_controldata when there is WAL', () => {
    ...
    expect(hostNote?.code).toContain('pg_controldata "./BackupForge-restore/18/docker"');
  ...
  it('builds the cluster at the version-specific path', () => {
    ...
    expect(combine18?.code).toContain('-o "./BackupForge-restore/18/docker"');
    expect(combine17?.code).toContain('-o "./BackupForge-restore/data"');
  });
  ...
  it('includes the WAL recovery step with a PGDATA-relative restore_command when there is WAL', () => {
    ...
    expect(recovery?.code).toContain('./BackupForge-restore/18/docker/recovery.signal');
    expect(recovery?.code).toContain('BackupForge_wal_restore/%f');
