...
export const buildManualSteps = ({
  ...
  if (hasWal) {
    ...
    steps.push({
      ...
      code: [
        `mkdir -p "${dataDir}/BackupForge_wal_restore"`,
        `for f in bundle/wal/*; do`,
        `  [ -e "$f" ] || continue`,
        `  case "$f" in`,
        `    *.zst) zstd -dq "$f" -o "${dataDir}/BackupForge_wal_restore/$(basename "\${f%.zst}")" ;;`,
        `    *) cp "$f" "${dataDir}/BackupForge_wal_restore/" ;;`,
        `  esac`,
        `done`,
        `{`,
        `  echo "restore_command = 'cp \\"BackupForge_wal_restore/%f\\" \\"%p\\"'"${targetLines}`,
