...
// the exact IP BackupForge connects from lives only in the server-side rejection, which the backend no
...
> = {
  [ConnectionErrorCode.PgHbaNoEntry]: {
    ...
    summary:
      'PostgreSQL has no pg_hba.conf entry that permits a replication connection from BackupForge. Physical backups stream via replication, which needs a "host replication" rule - an ordinary "host all" rule does not cover it.',
    buildSteps: () => [
      {
        type: 'note',
        runs: [
  ...
  [ConnectionErrorCode.SystemIdentifierMismatch]: {
    ...
    summary:
      'The cluster at this address has a different system identifier than the one this backup was set up for. Point BackupForge at the original cluster, or create a new database entry.',
  },
  [ConnectionErrorCode.ConnectionFailed]: {
    title: 'Could not connect',
    summary:
      'BackupForge could not open a replication connection. Check the host, port, SSL mode and that a firewall is not blocking the connection.',
  },
};
