...
- **Agent-mode.** Install a BackupForge-shipped binary on the same host as the
  ...
  user's behalf and coordinates with the central BackupForge over an outbound
  channel. This is the shape pgBackRest / Barman use, and the shape WAL_V1
  used inside BackupForge before it was deleted.
- **Remote-mode.** The BackupForge host itself runs `pg_basebackup` and
...
All BackupForge physical backup flows run **remotely** from the BackupForge
host against the user's PostgreSQL via the replication protocol. BackupForge
never ships a per-host agent. For users whose PG is not reachable from the
BackupForge host directly, the supported escape hatch is an **SSH tunnel**
from BackupForge to a jump host inside the user's perimeter — same backup
...
managed PG), reload, paste creds into BackupForge. Most managed PG providers
...
"BackupForge host runs `pg_basebackup` / `pg_receivewal` against a remote
...
container, point BackupForge at it, run the flow.

**3. Privacy and closed-network deployments are already covered.**
BackupForge is normally deployed inside the user's own perimeter — the
remote connection from BackupForge to PG crosses a private network the user
controls, not the public internet. For the minority of deployments where
PG sits behind a stricter inner perimeter than BackupForge itself, an SSH
tunnel from BackupForge to a jump host inside the PG perimeter delivers the
...
- **No agent release pipeline.** No second artifact to version, sign,
  ...
  BackupForge binary is the only thing the user runs.
...
- **Single test surface.** Physical CI exercises the same
  "spawn PG container, point BackupForge at it" matrix as logical, plus an
