...
BackupForge needs physical backups with incremental support and WAL streaming
...
flow BackupForge runs: `pg_basebackup` for FULLs, `pg_basebackup --incremental=`
...
- **PG 17+ requirement for physical.** Operators on PG 12–16 cannot use
  physical backups in BackupForge and stay on `pg_dump`. This is a real
...
- **Managed PG that forbids `ALTER SYSTEM`.** On a managed PG where
  ...
  provider's UI before BackupForge will save an incremental config. We
