# ADR-0012: pg_basebackup streams a single tar to stdout; backup_manifest is reconstructed on BackupForge
...
commits BackupForge to driving PG's native physical-backup binaries;
[ADR-0009](./0009-why-remote-physical-backups-instead-of-agents.md)
commits to running them remotely from the BackupForge host. Within those
...
teed on BackupForge into two independent branches: one uploads the bytes
...
PG→BackupForge link dominates, and compressing on the source sends ~1/3
of the bytes:
...
manifest field is derivable on BackupForge from the tar stream plus a
...
- PG→BackupForge traffic compressed ~3x. 1-3 TB backups complete in
...
- Net CPU on BackupForge to decompress and SHA-256 the full backup
